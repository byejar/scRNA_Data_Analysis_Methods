...
library(ggplot2)
  ...
  ggsave(fig_file)
######
load("/home/wus/2023-2-23_008_TCGAbiolinks_download/TCGA_RNA_data/TCGA-CESC.rda")
ls()
geneexp <- assay(data,i = "tpm_unstrand")
#TCGAquery_SampleTypes(barcode, typesample)
##  colnames()=  vector
...
##样本信息
sample_type_information<-colData(data)[ samples  ,c('sample_type','patient','project_id'),drop=FALSE]
df4<-merge(samples_express, sample_type_information, by = "row.names", all = F)

df5<-rbind(df3, df4, all = F)
df5<-df5[df5$sample_type != 'FALSE',]
p <-  ggboxplot(df5, x="project_id", y="ENSG00000168883.20", color = "sample_type",bxp.errorbar =TRUE,outlier.shape = NA)#,order=)
...
load("/home/wus/2023-2-23_008_TCGAbiolinks_download/TCGA_RNA_data/TCGA-CHOL.rda")
ls()
csv_file='/home/wus/2023-2-23_008_TCGAbiolinks_download/box/merge.csv'
geneexp <- assay(data,i = "tpm_unstrand")
#TCGAquery_SampleTypes(barcode, typesample)
##  colnames()=  vector
#### 选择是A的冰冻样本 
...
print(paste0('samplesNT:  ',samplesNT))
#R=TCGAquery_MatchedCoupledSampleTypes(b,c("NT","TP"))
...
write.csv( df3, csv_file,row.names=F,append = FALSE)
...
stat.test <- data4 %>% 
  group_by(project_id,fc) %>%
  t_test(ENSG00000168883.20 ~ sample_type) %>%
  adjust_pvalue(method = "bonferroni") %>% 
  add_significance("p.adj")

stat.test<-stat.test[order(stat.test$fc, decreasing = TRUE), ]
