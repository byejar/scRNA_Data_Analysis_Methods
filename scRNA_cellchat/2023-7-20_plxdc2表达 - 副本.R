...
save(mac_sce_rename,file = '/home/wus/for_seurat/8_1/immune_rename_2023-7-4.Rdata')
...
immune.combined.sct<-mac_sce_rename
DefaultAssay(immune.combined.sct) <- "SCT"
Idents(immune.combined.sct)="cell_type"

mac.cells <- subset(immune.combined.sct, idents = "T cell")
...
DimPlot(mac.cells, reduction = "umap", group.by = "seurat_clusters", label = TRUE,
    repel = TRUE)
FeaturePlot(mac.cells, features = c('Cd3e','Cd4','Cd8a','Foxp3','Mki67','Mzb1','Ncr1','Ncam1','Klrb1c'))
FeaturePlot(mac.cells, features =c('Cd3e','Cd4','Cd8a','Cd8b1','Top2a','Mki67','Klrb1c','Gzma'))
# ##差异表达基因
# pancreas.integrated.markers<- FindAllMarkers(mac.cells, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
# pancreas.integrated.markers %>%
#     group_by(cluster) %>%
...
T_ref_rename=mac.cells
new_id<-c('T_cell',"T_cell", "T_cell","NK",'T_cell')
names(new_id)<-levels(T_ref_rename)
T_ref_rename<-RenameIdents(T_ref_rename,new_id)

DimPlot(T_ref_rename,label = TRUE, 
...
T_ref_rename$celltype = as.character(Idents(T_ref_rename))
table(T_ref_rename$celltype )

sce$cell_type[match(colnames(T_ref_rename),colnames(sce))] =  T_ref_rename$celltype 
table(sce$cell_type)
...
save(sce,file = '/home/wus/for_seurat/8_1/immune_rename_2023-7-20.Rdata')